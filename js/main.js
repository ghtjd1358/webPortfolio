window.onload = function () {
  ...
  gsap.timeline({
    scrollTrigger: {
      ...
      // scrub은 scrollTrigger의 이벤트가 스크롤이 사용될때만 재생되도록 만들어 주는 속성 (안적으면 트리거시점 나오면 스크롤안해도 계속 애니메이션 진행됨)
      // scrub은 true나 숫자로 값을 써줄수있는데 true 같은 경우는 스크롤하면 애니메이션 바로 진행되고, 바로 멈추며 숫자는 그 시점을 따라잡는데 N초가 걸려서 애니메이션이 더 부드러움
      scrub: 1,// 스크롤 할때만 적용 true, 1,2,3 숫자는 초
      // markers: true
    }
  })
  ...
  gsap.utils.toArray('.subText p').forEach((selector) => {
    gsap.timeline({
      ...
      .fromTo(selector, { opcity: 0, y: 100 }, { opacity: 1, y: 0, ease: 'none', duration: 5 }, 0);
  });

  //04. con1 TextAni 텍스트체인지 gsap 애니메이션
...
//07. con5 listBox li호버시 이미지 보이는 애니메이션
...
for (let i = 0; i < listBox.length; i++) {
  ...
  listBox[i].addEventListener('mouseover', (e) => {
